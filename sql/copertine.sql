...
--  =====  COME SI ESEGUE  ============================================
--
--  Un blocco alla volta, incollandolo da solo nella scheda SQL.
--
--  Niente controlli preliminari su information_schema: phpMyAdmin si
--  collega come utente cPanel, che lì dentro su questo hosting non può
--  guardare (#1044). Quindi qui ci sono ALTER e CREATE semplici, e il
--  controllo lo fa MySQL a posteriori:
--
--    #1060  Nome colonna duplicato   → quel blocco era già stato fatto
--    #1061  Nome chiave duplicato    → idem
--
--  Sono conferme, non guasti: salta al blocco dopo. Ma phpMyAdmin si
--  ferma al primo errore, ed è per questo che i blocchi vanno lanciati
--  separatamente invece che tutti insieme.
-- =====================================================================


-- --- BLOCCO 1 — il catalogo ------------------------------------------
-- Va per primo perché è l'unico che non può fallire: se la tabella c'è
-- già, IF NOT EXISTS non fa niente e non interrompe niente.
...
-- --- BLOCCO 2 — le colonne sugli articoli ----------------------------
...
-- --- BLOCCO 3 — l'indice --------------------------------------------
-- Serve solo a rendere svelta la coda di copertine.php. Se dà #1061
-- c'era già, e va bene così.
